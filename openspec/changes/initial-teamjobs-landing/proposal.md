...
- Config knobs: verified local legal routes and WhatsApp number (default `+5215610275879`); unresolved login/social destinations are omitted and the Contact form has no endpoint or transport configuration
...
- Pixel-perfect mobile (no mockups — standard responsive interpretation)
...
- `site-navigation`: navbar (desktop + mobile island), footer columns, in-page anchors, verified legal routes, WhatsApp FAB
...
| `src/pages/`, `src/layouts/`, `src/components/**`, `src/islands/**` | New | Landing, legal routes, sections, footer, and islands |
...
| Unresolved external destinations | Med | Omit login/social actions until destinations are verified |
...
| Historical PR1 generated lockfile | High (historical) | The original 400-total-line baseline required the sole approved `size:exception` for PR1's generated lockfile. This exception is historical only; current remaining slices use the 800-authored-changed-line ceiling and the present 782-line slice has 18 lines of headroom with no new exception. |
...
- [ ] Each current remaining delivery slice ≤800 authored changed lines; the current 782-line slice has 18 lines of headroom, while historical PR1's generated-lockfile `size:exception` remains an audit fact and is not reused
